...
from aDCF_metric import calculate_adcf_soft_act, calculate_adcf_hard_min
import os
...
sys = 'wBCE' # This can be either 'wBCE' (aDCF loss with BCE) or 'woBCE' (aDCF loss without BCE). In the paper, we used as 'wBCE'
epoch_size = 10
bs = 1024

folder_path = "results/" + str(model.name) + "_" + sys + "_thresOpt_" + str(bs) + "_" + str(epoch_size) + "_" + str(lr)
...
f2 = open(folder_path+"/text_files/bestThreshold.txt", "w")
...
for epoch in range(epoch_size):
    ...
    preds_trn, keys_trn = calculation(model, trn_loader)
    
    trn_far_asvs_hard, trn_far_cms_hard, trn_frrs_hard, trn_fars_hard, trn_a_dcfs_hard, trn_a_dcfs_hard_min, trn_a_dcf_thresh_hard, \
    trn_a_dcf_thresh_hard_min, trn_far_asvs_hard_min, trn_far_cms_hard_min, trn_frrs_hard_min = calculate_adcf_hard_min(preds_trn, keys_trn)
    
    preds_dev, keys_dev = calculation(model, dev_loader)
    
    dev_far_asvs_soft_act, dev_far_cms_soft_act, dev_frrs_soft_act, dev_adcf_soft_act = calculate_adcf_soft_act(preds_dev, keys_dev, trn_a_dcf_thresh_hard_min)
    ...
    if dev_adcf_soft_act[0] < adcf:
        ...
        best_epoch = epoch+1
        best_threshold = trn_a_dcf_thresh_hard_min
        threshold = trn_a_dcf_thresh_hard_min
        print(f'\nEpoch-{epoch+1} min aDCF: {adcf:.6f} Threshold: {threshold:.6f}')
    
    
    
f2.writelines('Epoch: {} min_aDCF: {} Threshold: {}\n'.format(best_epoch, adcf, best_threshold))
f1.close()
f2.close()

plt.plot(range(epoch_size), all_thres, color='red', label='Threshold')
plt.xlabel('Epoch')
plt.ylabel('Threshold')
plt.title('Threshold with respect to epoch')
plt.legend()
plt.savefig(folder_path+"/figures/threshold.png")
plt.show()

plt.plot(range(epoch_size), trn_loss, color='red', label='Total Train Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Total Train Loss with respect to epoch')
plt.legend()
plt.savefig(folder_path+"/figures/total_train_loss.png")
plt.show()

plt.plot(range(epoch_size), adcf_losses, color='red', label='aDCF Train Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('aDCF Train Loss with respect to epoch')
plt.legend()
plt.savefig(folder_path+"/figures/adcf_train_loss.png")
plt.show()

if sys == 'wBCE':
    plt.plot(range(epoch_size), bce_losses, color='red', label='BCE Train Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('BCE Train Loss with respect to epoch')
    plt.legend()
    plt.savefig(folder_path+"/figures/bce_train_loss.png")
    plt.show()
